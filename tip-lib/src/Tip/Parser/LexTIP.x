...
@rsyms = \( | \) | "declare" \- "datatype" | "declare" \- "datatypes" | "declare" \- "sort" | "declare" \- "const" | "declare" \- "fun" | "define" \- "fun" | "define" \- "fun" \- "rec" | "define" \- "funs" \- "rec" | "set" \- "logic" | "check" \- "sat" | "assert" \- "claim" | \= \> | \- | \_ | \@ | \= | \+ | \* | \/ | \> | \> \= | \< | \< \=
...
-- token UnquotedSymbol
([\! \$ \% \& \* \+ \. \/ \< \= \> \? \@ \^ \_ \~]| $l)([\! \$ \% \& \* \+ \- \. \/ \< \= \> \? \@ \^ \_ \~]| ($d | $l)) *
    { tok (eitherResIdent T_UnquotedSymbol) }

-- token QuotedSymbol
\| ([$u # \|]| \\ $u)* \|
    { tok (eitherResIdent T_QuotedSymbol) }

-- token Keyword
\: (\- | ($d | $l)) *
    { tok (eitherResIdent T_Keyword) }
...
resWords =
  b "declare-datatypes" 24
    (b ">=" 12
       ...
       (b "and" 18
          (b "Int" 15
             (b "Bool" 14 (b "@" 13 N N) N) (b "_" 17 (b "Real" 16 N N) N))
          (b "check-sat" 21
             (b "assert-claim" 20 (b "assert" 19 N N) N)
             (b "declare-datatype" 23 (b "declare-const" 22 N N) N))))
    (b "lambda" 36
       (b "distinct" 30
          (b "define-fun" 27
             (b "declare-sort" 26 (b "declare-fun" 25 N N) N)
             (b "define-funs-rec" 29 (b "define-fun-rec" 28 N N) N))
          (b "false" 33
             (b "exists" 32 (b "div" 31 N N) N)
             (b "ite" 35 (b "forall" 34 N N) N)))
       (b "par" 42
          (b "mod" 39
             (b "match" 38 (b "let" 37 N N) N) (b "or" 41 (b "not" 40 N N) N))
          (b "to_real" 45
             (b "set-logic" 44 (b "prove" 43 N N) N) (b "true" 46 N N))))
  where
  b s n = B bs (TS bs n)
    where
    bs = s
