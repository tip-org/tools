...
newInfo :: (Pretty a, Ord a, Monad m) => a -> ScopeT a m ()
newInfo x = do
  s <- gets inner
  case S.member x s of
    True ->
      throwError $
        fsep [text "Info", pp x, text "rebound"]
    False ->
      modify (\s -> s { inner = S.insert x (inner s) })

newTyVar :: (Monad m, Ord a, Pretty a) => a -> ScopeT a m ()
newTyVar ty = do
  newInfo ty
  modify $ \s -> s {
    types = M.insert ty TyVarInfo (types s) }

newAbsType :: (Monad m, Ord a, Pretty a) => AbsType a -> ScopeT a m ()
newAbsType (AbsType ty) = do
  newInfo ty
  modify $ \s -> s {
    types = M.insert ty AbsTypeInfo (types s) }

newDatatype :: (Monad m, Ord a, Pretty a) => Datatype a -> ScopeT a m ()
...
newConstructor :: (Monad m, Ord a, Pretty a) => Datatype a -> Constructor a -> ScopeT a m ()
...
newFunction :: (Monad m, Ord a, Pretty a) => AbsFunc a -> ScopeT a m ()
newFunction AbsFunc{..} = do
  newInfo abs_func_name
  modify $ \s -> s {
    globals = M.insert abs_func_name (FunctionInfo abs_func_type) (globals s) }

newLocal :: (Monad m, Ord a, Pretty a) => Local a -> ScopeT a m ()
newLocal Local{..} = do
  newInfo lcl_name
  modify $ \s -> s {
    locals = M.insert lcl_name lcl_type (locals s) }

withTheory :: (Monad m, Ord a, Pretty a) => Theory a -> ScopeT a m b -> ScopeT a m b
