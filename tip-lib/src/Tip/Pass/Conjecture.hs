...
skolemiseConjecture' thy =
  case goals of
    [Formula Prove name src i tvs body] ->
      case tvs of
        [] -> do let (body',(sks,pre)) = runWriter (skolemise body)
                 ...
                 return thy {
                       ...
                       thy_asserts = Formula Prove name src i [] body'' : map (formula Assert Nothing Nothing []) pre'' ++ assums
                     }
  ...
  formula r n src tvs (Quant (QuantIH i) q vs e) = Formula r n src (IH i) tvs (mkQuant q vs e)
  formula r n src tvs e = Formula r n src Unknown tvs e 
...
negateConjecture = fmap (declsPass (map neg1)) . typeSkolemConjecture
  where
  neg1 (AssertDecl (Formula Prove n src i [] form))
      = AssertDecl (Formula Assert n src i [] (gentleNeg form))
  neg1 d0 = d0
...
typeSkolemConjecture thy =
  ...
  where
  isProve (Formula Prove _ _ _ _ form) = True
  isProve _ = False

  ty_skolem1 thy (Formula Prove n src i tvs form) = do
    tvs' <- mapM (refreshNamed "sk_") tvs
    return thy {
      thy_asserts = Formula Prove n src i [] (makeTyCons (zip tvs tvs') form):thy_asserts thy,
